...
<div class="card-header">{{$profile->name}}</div>
	...
	<div class="col-xl-12" style="text-align:center;">
		<!--follo-->
	<form action="{{route('profile.like')}}" method="post">@csrf
	<input type="hidden" name="profiles_like" value="{{$profile->id}}">
		<button class="btn btn-outline-primary" data-toggle="tooltip" data-placement="top" title="Click to follow" type="submit">{{$likers}} Likes</button>
